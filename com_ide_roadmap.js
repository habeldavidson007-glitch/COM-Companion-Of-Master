...
const children = [
  ...
  callout('What COM IDE does that nothing else does', [
    ...
    '5.  Runs inference locally on Ollama - optimized for 2-3GB RAM systems, works without internet',
  ], P.accent3, P.accent),
  ...sp(1),
  ...
  phaseCard('1', 'Project Scanner + Error Explainer', '6-8 weeks', 'START HERE', [
    ...
    'CRITICAL: Must run on 2-3GB RAM total - use smollm2:1.7b (1.2GB) as primary model, load larger models only on-demand then release',
  ...
  phaseCard('2', 'Context-Aware Q&A + Memory', '6-8 weeks', 'FIRST DEMO', [
    ...
    'RAM constraint: Use context compression to keep project index under 500MB, unload model after 5 minutes idle',
  ...
  phaseCard('3', 'VS Code Extension', '8-10 weeks', 'DISTRIBUTION', [
    ...
    'Lightweight design: extension host runs minimal logic, heavy lifting done by background COM service with strict RAM limits',
  ...
  h1('05  Hardware Reality - Building for 2-3GB RAM'),
  body('This is the defining constraint of COM IDE. Most AI tools assume 16GB+ systems. COM IDE is built for developers on low-end hardware - because they are the ones who feel the pain of silent crashes and have no other options. Here is exactly what works and how.'),
  ...sp(1),
  callout('The 2-3GB RAM Design Philosophy', [
    'You are not building a worse version of a 16GB tool. You are building a different tool entirely - one that respects memory as the primary scarce resource.',
    'Every component must justify its RAM usage. If a feature cannot work within the budget, it is not "optimized later" - it is redesigned from the start.',
    'This constraint is a competitive advantage: you will ship a product that runs where competitors cannot.',
  ], P.accent3, P.accent),
  ...sp(1),
  new Table({
    ...
    rows:[
      ...
      ...[['smollm2:1.7b-q4_K_M', '~1.2 GB', '~45 tok/s', 'PRIMARY MODEL - intent routing, fast replies, always loaded'], ['qwen2.5-coder:3b-q4', '~2.0 GB', '~25 tok/s', 'On-demand only - code generation, load then release'], ['tinyllama:1.1b-q4', '~700 MB', '~60 tok/s', 'Fallback when RAM critically low'], ['phi3-mini:3.8b-q4', '~2.4 GB', '~22 tok/s', 'Alternative 3B tier (tight but viable)'],
  ...
  ...sp(1),
  callout('Recommended Architecture for 2-3GB Systems', [
    'Base footprint: smollm2:1.7b always loaded (~1.2GB). This handles intent routing, simple Q&A, and signal dispatch.',
    'Burst capacity: Load qwen2.5-coder:3b ONLY when generating code or explaining complex errors (~2.0GB additional). Release immediately after response completes.',
    'Context compression: Project index must stay under 300MB using TF-IDF with aggressive pruning. No full-document caching.',
    'Godot + OS budget: Leave 800MB-1GB for Godot engine and OS overhead. This is non-negotiable - if COM uses more, the system swaps and everything slows.',
    'Total peak usage: 1.2GB (base model) + 0.3GB (index) + 0.3GB (COM runtime) = 1.8GB base. Burst to 3.8GB during code gen, then back to 1.8GB within 30 seconds.',
  ], P.blue_bg, P.blue),
  ...sp(1),
  callout('Critical Techniques That Make This Work', [
    '1. Model hot-swapping: Never keep two models loaded simultaneously. Use a model manager that unloads before loading the next.',
    '2. Response streaming: Stream tokens as they arrive so user sees progress even on slow CPU inference. Perceived latency matters more than actual latency.',
    '3. Query classification: Route simple queries to smollm2, complex queries to 3B model. Do not use the 3B model for "what nodes exist in MainScene?"',
    '4. Aggressive idle timeout: Unload large model after 2 minutes of inactivity, not 10 minutes. User waits 3 seconds to reload vs wasting 2GB RAM for 8 minutes.',
    '5. Context window discipline: Keep context under 2048 tokens for smollm2, under 4096 for 3B model. Larger contexts = more RAM + slower inference.',
  ], P.gold_bg, P.gold),
  ...sp(1),
  callout('CPU Inference Reality Check', [
    'On a modern i5/Ryzen 5: smollm2:1.7b runs at 40-50 tok/s, qwen2.5-coder:3b runs at 20-28 tok/s.',
    'A 150-token error explanation takes 3-4 seconds on smollm2, 6-8 seconds on 3B model. This is acceptable for debugging workflows.',
    'No GPU required. A GPU would improve speed 3-4x but is not necessary for functionality. Target CPU-first, GPU-is-a-bonus.',
  ], P.accent3, P.accent),
  ...sp(1), pgBreak(),
  ...
  bul([bold('Hardware bottleneck: '), run('2-3GB RAM is tight but workable. The risk is model loading failures if Godot also needs RAM. Mitigation: implement graceful degradation - if large model fails to load, fall back to smollm2 with a message "simplified response due to memory constraints".')]),
  bul([bold('Scope creep: '), run('The E+ language is exciting. The standalone IDE is exciting. Phase 1 is unglamorous file parsing. The discipline to finish Phase 1 before touching Phase 4 is the single most important non-technical decision in this project.')]),
  bul([bold('Model quality on small models: '), run('smollm2:1.7b is fast but less capable than 7B+ models. Mitigation: use it for routing and simple Q&A only. For code generation, always burst to 3B model even with the latency cost. Better to wait 8 seconds for correct code than 3 seconds for wrong code.')]), ...sp(1), pgBreak(),
  ...
  body('The people who will use this are sitting in Discord right now asking why their player keeps falling through the floor. Build for them. Build for the developers who cannot afford 16GB RAM machines but still deserve tools that understand their code.'), ...sp(2),
  rule(P.accent),
